...
<body>
    ...
    <script>
        ...
        async function generateAnalysis(apiKey, homeTeam, awayTeam) {
            const prompt = `คุณเป็นนักวิเคราะห์ฟุตบอลมืออาชีพ กรุณาวิเคราะห์แมตช์ระหว่าง "${homeTeam}" (ทีมเหย้า) vs "${awayTeam}" (ทีมเยือน) และ DeeperSearch เพื่อค้นหาข้อมูลล่าสุดเกี่ยวกับแมตช์ระหว่าง "${homeTeam}" (ทีมเหย้า) vs "${awayTeam}" (ทีมเยือน) เช่น ข่าวการบาดเจ็บ, ฟอร์มล่าสุด, สถิติการพบกัน, และอัตราต่อรองการเดิมพันจากแหล่งที่น่าเชื่อถือ เช่น Sky Sports, BBC Sport, หรือ Polymarket

กรุณาคัดกรองและสรุปข้อมูลจาก DeeperSearch และเว็บเพื่อให้การวิเคราะห์แม่นยำที่สุด โดยวิเคราะห์ตามหัวข้อเหล่านี้:
